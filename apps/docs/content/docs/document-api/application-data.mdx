...
async function attachFinding() {
  ...
  // per segment. Nothing in the public selection shape reports run boundaries,
  // so the catch below is the only place that case can be handled — the same
  // is true of a selection crossing a table-cell or section boundary.
  ...
  // this action runs), when a segment crosses two `w:t` runs, when segments
  // are not contiguous paragraphs (or cross a table-cell/section boundary),
  // and when a paragraph is not in the main document part.
  //
...
<Callout variant='warning' title='The range comparison narrows the risk without closing it'>
  ...
  dialog, so any author or any other tool can set it. Same-tag controls are grouped by document position into the
  anchor's segments, so a control carrying that alias and a colliding tag near the real anchor can be absorbed as an
  extra segment (or, if it breaks the contiguous run, excluded and resolved as a separate, wrong match) — either way,
...
The single-run requirement is the one that surfaces most often, because a bold or italic word splits a paragraph into several runs and a selection across that boundary throws even though it looks like ordinary text in one paragraph — this applies per segment, so it surfaces just as often inside a multi-paragraph target.
