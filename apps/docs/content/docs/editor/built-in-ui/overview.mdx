...
covers `ui.contentControls`. **Layout** covers `ui.ruler` alongside the Editor's zoom and responsive shell. **Loading**
covers the progress UI shown while a document opens.
