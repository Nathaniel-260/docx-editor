...
happens when a person activates a link. **Context menu** covers right-click actions. **Content controls** covers
structured fields. **Ruler** covers left and right page margins. **Responsive layout** covers zoom and the Editor shell.
**Loading** covers the progress UI shown while a document opens.
