...
Start with [Build your first custom control](/editor/custom-ui/controller-setup). Its Vanilla and React examples both use
...
Choose a sample selection below to see the state a custom Bold control receives. This model is simulated so you can
compare enabled, active, and disabled states without finding each one in a DOCX.
...
Map stable `reason` values to product language. Keep a fallback so newer reasons remain understandable before the application adds tailored copy:

<include>../../../../snippets/editor/command-failures.ts</include>

Use the state fields together. `enabled: false` means the action cannot run now; `supported: false` means the controller cannot route it on this host. State is a snapshot rather than a guarantee, so execution still needs its own result check.
...
Branch on stable reason strings and receipt failure codes. Show the supplied message to developers or adapt it for users, but keep a fallback for codes the application does not recognize yet.

For document-operation capability checks and receipt failure codes, read [Receipts and errors](/document-api/receipts-and-errors).
...
Continue with [Formatting controls](/editor/custom-ui/formatting-controls), or compare custom controls with the
[configured built-in toolbar](/editor/built-in-ui/configure-the-toolbar).
