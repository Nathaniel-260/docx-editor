...
Editing is available at the `write` and `resolve` comment levels. Only `resolve` adds the resolve and reopen actions. The
`read` level refuses both kinds of mutation.
...
The panel disables its write controls in viewing mode, which it reads from the public document slice. When your
application configures `interaction.comments.level`, use that same application-owned value to decide which controls to
render. The controller still refuses forbidden mutations, so always inspect its receipt.

[Build your first custom control](/editor/custom-ui/controller-setup) covers the React provider and binding lifecycle this
example assumes.
