...
Content controls are structured fields stored in a DOCX. Use `superdoc.ui.contentControls` to keep an application-owned
panel synchronized with the Editor. Use the Document API to change a control's content or properties.

If SuperDoc should render the control chrome instead, see [Content controls](/editor/built-in-ui/content-controls).
