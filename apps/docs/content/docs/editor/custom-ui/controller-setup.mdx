...
- `hyperlinks: false` suppresses hyperlink activation.
...
These are rendering switches, not permissions. `ui: { comments: false }` hides the built-in comments interface but does not stop your own panel from creating comments; `interaction: { comments: { level: 'read' } }` is what refuses the write.
...
Build the next control with the same state-and-command pattern, or return to [Choose your interface](/editor/who-renders-the-ui) before expanding the toolbar.
