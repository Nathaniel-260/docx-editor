...
V1 applications supplied an external Y.Doc and provider through `modules.collaboration`. V2 instead takes a supported provider target on `document.v2Collaboration` and owns the provider lifecycle. Do not port the external pair directly.

[Connect to a collaboration room](/editor/collaboration) owns the provider shapes, room modes, exception codes, readiness, and cleanup contract. Use [Upgrade a document](/editor/collaboration/upgrade-a-document) when a local Editor should create a new room without remounting.
