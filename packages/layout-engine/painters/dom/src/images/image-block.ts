...
export const createBlockImageContent = ({
  ...
  if (!block.src && block.placeholder) {
    const placeholder = createRenderPlaceholder({ doc, placeholder: block.placeholder });
    if (className) placeholder.classList.add(className);
    placeholder.style.display = imageDisplay ?? (block.display === 'inline' ? 'inline-flex' : 'flex');
    placeholder.style.width = '100%';
    placeholder.style.height = '100%';
    return buildImageHyperlinkAnchor?.(placeholder, block.hyperlink, hyperlinkDisplay) ?? placeholder;
  }
